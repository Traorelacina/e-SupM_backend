...
class AdminProductController extends Controller
{
    ...
    {
        $q = Product::with(['category:id,name', 'productCategory:id,name', 'primaryImage']);
        ...
        if ($request->product_category_id) $q->where('product_category_id', $request->product_category_id);
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $product = DB::transaction(function () use ($request, $data) {
                ...
                return $product->load(['images', 'category', 'productCategory']);
            });
            ...
            return response()->json([
                'success' => true,
                'message' => isset($data['is_draft']) && $data['is_draft']
    ...
    public function show(int $id): JsonResponse
    {
        $product = Product::withTrashed()
            ->with(['category', 'productCategory', 'images', 'partner', 'reviews'])
            ->findOrFail($id);
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        try {
            ...
            $numericFields = [
                ...
                'stock', 'low_stock_threshold', 'admin_label_discount',
            ];
            ...
            // Nullable foreign key — permettre de désassocier la catégorie produit
            if ($request->has('product_category_id')) {
                $val = $request->input('product_category_id');
                $data['product_category_id'] = ($val === '' || $val === null || $val === 'null') ? null : (int)$val;
            }
            ...
            return response()->json([
                ...
                'data'    => $product->fresh()->load(['category', 'productCategory', 'images']),
            ]);
    ...
    public function toggle(int $id): JsonResponse
    {
        try {
            ...
            if ($newStatus) {
                $product->update(['is_active' => true, 'is_draft' => false]);
            } else {
    ...
    public function lowStock(): JsonResponse
    {
        $products = Product::lowStock()
            ->with(['category:id,name', 'productCategory:id,name', 'primaryImage'])
            ->get();
    ...
    public function export(): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        ...
        return response()->streamDownload(function () {
            ...
            fputcsv($handle, [
                ...
                'stock', 'category', 'product_category', 'brand', 'is_active', 'is_draft',
            ]);

            Product::with(['category:id,name', 'productCategory:id,name'])->chunk(100, function ($products) use ($handle) {
